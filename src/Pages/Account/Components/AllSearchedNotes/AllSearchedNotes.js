...
function AllSearchedNotes() {
    ...
    const [allNotes, loading, setUrl] = useNotes(`https://note-taking-server.netlify.app/.netlify/functions/app/get-notes/search:${search.toLowerCase()}`);
    ...
    useEffect(() => {
        setUrl(`https://note-taking-server.netlify.app/.netlify/functions/app/get-notes/search:${deferredSearch.toLowerCase()}`);
    }, [deferredSearch])
